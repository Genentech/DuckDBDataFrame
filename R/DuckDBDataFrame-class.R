...
#'     \code{"integer"}, \code{"integer64"}, \code{"double"},
...
#' }
#'
#' @include DuckDBAtomicList-class.R
#' @include DuckDBColumn-class.R
#' @include DuckDBEmbeddings-class.R
...
setMethod("[[", "DuckDBDataFrame", function(x, i, j, ...) {
    ...
    i <- normalizeDoubleBracketSubscript(i, x, allow.nomatch = TRUE)
    if (is.na(i))
        return(NULL)

    column <- extractCOLS(x, i)
    table <- as(column, "DuckDBTable")

    # Get BiocDuckDB type string for dispatch
    schema <- .get_duckdb_schema(table@conn, table@datacols)
    colname <- names(table@datacols)[1L]
    coltype <- .duckdb_container_type(schema[[colname]])
    if (grepl("^list<.*>$", coltype)) {
        .new_DuckDBAtomicList(table, coltype, metadata = as.list(mcols(column)))
    } else if (grepl("^array<(double|float|real|decimal),\\d+>$", coltype)) {
        .new_DuckDBEmbeddings(table, coltype, metadata = as.list(mcols(column)))
    } else {
        new2("DuckDBColumn", table = table, metadata = as.list(mcols(column)), check = FALSE)
    }
})
...
setAs("DuckDBDataFrame", "DFrame", function(from) {
    df <- as(as.data.frame(from, optional = TRUE), "DFrame")
...
makePrettyCharacterMatrixForDisplay <- function(x) {
    ...
    if (x_nrow <= nhead + ntail + 1L) {
        ...
        if (ntail == 0L) {
            ...
            df <- DataFrame(as.data.frame(x[i, , drop = FALSE], optional=TRUE),
                            check.names = FALSE)
...
setMethod("makeNakedCharacterMatrixForDisplay", "DuckDBDataFrame", function(x) {
    callGeneric(DataFrame(as.data.frame(x, optional = TRUE),
                          check.names = FALSE))
})
